...
2. The goal of the setup process is to create a virtual environment and install necessary packages for running the notebooks. The required packages as defined in the `requirements_recommender.txt` file will be installed in the environment. There are two ways to accomplish this, so you can choose the method that works best for you:
    ...
    2. **Manually** - following the steps below in the command line:
        ...
        3. `cd` out back to the repository root folder, and activate the virtual environment by running:
            * Linux or Mac: `source ./venv/recommender/bin/activate` 
            * Windows CMD: `.\venv\recommender\Scripts\activate.bat`
            * Windows Powershell: `.\venv\recommender\Scripts\Activate.ps1`
         4. Install the packages: `pip install -r requirements_recommender.txt`
         5. Install `ipykernel`: `python -m ipykernel install --user --name=recommender --display-name=recommender`
...
* Then, open a notebook and set the right kernel(`recommender`). Setting the kernel is done on the top right side in `vscode`. 
...
### Testing the Setup
To make sure that the environment, and the `vscode` integration were all set up correctly - open the file `setup/test_setup.ipynb`, set the right kernel(`recommender`), and run. If you get a `succeeded` message - these modules are working fine. 